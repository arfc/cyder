...
MatDataTablePtr MaterialDB::initializeFromSQL(string mat) {
    std::vector<StrList> znums = db_->query("SELECT Z FROM "+mat);
    std::vector<StrList> dnums = db_->query("SELECT D FROM "+mat);
    std::vector<StrList> knums = db_->query("SELECT K_d FROM "+mat);
    std::vector<StrList> snums = db_->query("SELECT S FROM "+mat);
