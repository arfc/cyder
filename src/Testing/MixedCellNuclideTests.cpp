...
TEST_F(MixedCellNuclideTest, transportNuclidesDRhalf){ 
  ...
  // Neumann
  double expected_neumann= (mat_table_->D(u_))*expected_conc/(outer_radius*2 - mixed_cell_ptr_->geom()->radial_midpoint());
  ...
  EXPECT_FLOAT_EQ(expected_conc, nuc_model_ptr_->dirichlet_bc(u235_));
  // Neumann 
  expected_neumann= (mat_table_->D(u_))*expected_conc/(outer_radius*2 - mixed_cell_ptr_->geom()->radial_midpoint());
  EXPECT_FLOAT_EQ(expected_neumann, nuc_model_ptr_->neumann_bc(zero_conc_map, outer_radius*2, u235_));
  // Cauchy
