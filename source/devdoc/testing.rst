...
The Cyder Unit Tests 
====================

Introduction
------------

The *Cyder* provide the `basic guarantee 
...
*Cyder* makes use of the `Google Test 
<http://code.google.com/p/googletest/>`_ framework. 
It is a tool which will assist Cyder developers in incorporating tests for their 
...
behavior.  A Cyder unit test should: 
...
Cyder test coverage should mimic the hierarchical structure of the source code.
...
Cyder objects such as Materials, Commodities, Markets, Facilities and Messages. 
...
it is intended to be satisfy, the Cyder core developers have created 
parameterized abstract interface tests that must be instantiated with your 
concrete model in order for it to be accepted into the cyder module library.
...
FacilityModelTests.cpp is linked to the CyderUnitTestDriver executable, and 
...
Test Cases are logical groupings of Tests. For Cyder, each class, such as the 
...
The Test Case is a class declared in this file. The Cyder convention is to give 
...
Tests are performed on single units of functionality within Cyder. For Cyder, 
...
*Cyder* relies on reading xml files to initialize modules. It is 
...
XML node and context must be provided. Normally in *Cyder*, the
